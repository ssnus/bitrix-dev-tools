# 🛠 Инструменты разработчика для 1С-Битрикс

Панель управления для ускорения разработки на Битрикс. Позволяет управлять кэшем, включать режим отладки и просматривать логи в один клик.

## ✨ Возможности

- 🗑 **Очистка кэша**: Выборочная (компоненты, меню, HTML) или полная
- 🚫 **Отключение кэша**: Временная остановка записи кэша для разработки
- 🟢 **DEV-режим**: Вывод PHP-ошибок на экран
- ⚙️ **Агенты**: Принудительный запуск отложенных задач
- 📋 **Логи**: Просмотр последних ошибок прямо в админке
- 👤 **Инфо**: Данные о текущей сессии администратора

## 📋 Требования

- 1С-Битрикс: Управление сайтом (любая редакция)
- PHP 7.4 или выше

## 🚀 Установка
## 📥 Установка через консоль (curl)

Если вы предпочитаете командную строку, модуль можно установить без скачивания архива вручную.

### Быстрая установка

Выполните последовательно следующие команды в терминале (находясь в корне сайта):

```bash
# 1. Переходим в папку для модулей
cd local/modules/

# 2. Скачиваем архив с GitHub (замените ТВОЙ-НИК на ваш никнейм)
curl -L https://github.com/ssnus/bitrix-dev-tools/archive/refs/heads/main.zip -o dev.tools.zip

# 3. Распаковываем архив
unzip dev.tools.zip

# 4. Переименовываем папку в нужное имя модуля
mv bitrix-dev-tools-main dev.tools

# 5. Удаляем архив
rm dev.tools.zip
```
### Далее: Через админку (рекомендуется)

1. Скачайте архив с релизом
2. Распакуйте в папку `/local/modules/dev.tools/`
3. В админке перейдите: **Marketplace → Установленные решение**
4. Найдите **«Инструменты разработчика»** и нажмите **Установить**
5. ✅ Модуль автоматически создаст необходимые файлы
6. Меню появится в разделе **Сервисы → Dev Tools**

### Вариант 2: Вручную (если автоустановка не сработала)

Если после установки меню не появилось, создайте файл-прокси:

**Файл:** `/bitrix/admin/dev_tools.php`

```php
<?php
require($_SERVER["DOCUMENT_ROOT"] . "/local/modules/dev.tools/admin/dev_tools.php");
```
## 🎯 Использование

После установки в админке появится новая панель:

1. **Управление кэшем**: Включайте/выключайте кэш или очищайте выборочно
2. **Режим отладки**: Показывает ошибки PHP на странице
3. **Логи**: Быстрый просмотр `bitrix_error.log`

> ⚠️ **Важно:** Не оставляйте DEV-режим включённым на рабочем сайте!


## 🤝 Contributing

Если нашли баг или есть идея для улучшения — создавайте Issue или Pull Request!
